/* router used to handle multiple responses at once
 * dealer used to handle multiple requests at once
 */
'use strict';
const
...
if (cluster.isMaster) {
  ...
  router.on('message', function() { // response is ready to send
    let frames = Array.prototype.slice.call(arguments);
    dealer.send(frames);
  });

  dealer.on('message', function() { // after request is made
    let frames = Array.prototype.slice.call(null, arguments);
    router.send(frames); // send split up requests to router
  });
  ...
  // worker for each request (3 in this case)
  for (let i = 0; i < 3; i++) {
    cluster.fork();
  }
